...
const Swiper: FC<IProps> = (props: IProps): ReactElement => {
        ...
        timer.current = setInterval(() => {
            setCurIndex(curIndex => {
                ...
                if (curIndex === picList.current.children.length - 1) {
                    return picNav.current.children.length - 1;
                }
                return curIndex + 1;
            })
        }, 3000)
    ...
    useEffect(() => {
        ...
        setShiftX(shiftX => {
            ...
            return -curIndex * picList.current.parentElement!.clientWidth;
        })
    }, [curIndex])
    ...
    useEffect(() => {
        ...
        Array.from(picNav.current.children).forEach((item) => {
            item.classList.remove("active");
        })
        Array.from(picNav.current.children)[
            curIndex % Array.from(picNav.current.children).length
    ...
    const handleEnd = (e: React.TouchEvent<HTMLUListElement>) => {
        ...
        if (Math.abs(differ) > 0.3 * picList.current.parentElement!.clientWidth) {
            setCurIndex(curIndex => curIndex - differ / Math.abs(differ));
        } else {
            setShiftX(-curIndex * picList.current.parentElement!.clientWidth);
        }
        picList.current.style.transition = "0.5s";
        autoPlay();
    }
            ...
            >
                {
                    props.dataSource.map((item) => {
                        return (
                            <li key={item.id}>
                                <Link to={`/buy/${item.id}`}>
                                    <img src={item.icon} alt="tab_picture" />
                                </Link>
                            </li>
                        )
                    })
